...
# Boot-process presence path for a spawned (team, agent). The boot script
# `spawn.sh` writes (spawn.sh, the boot script it generates) writes its own
# pid here right before exec'ing the role's CLI, and removes it right after
# that CLI exits. Present iff a launcher-spawned CLI process for that role is
# currently running -- independent of the ready sentinel above, which only
# appears once the CLI/model reaches its own watcher-registration step. A
# type whose actas flow is model-driven (e.g. grok-build's watcher start,
# see docs/adr/0003) can get permanently stuck before that step -- most
# visibly when the underlying model call itself fails (a rate/usage-limit
# dialog the model never resolves) -- leaving no watcher and no ready
# sentinel even though the CLI process is still alive on-screen. This file
# lets an external liveness check (or a future spawn/status command) tell
# "process launched and still running, registration incomplete" apart from
# "nothing was ever launched, or it has exited." Same encoding as the lock
# path.
agmsg_boot_pid_path() {
  local team="$1" agent="$2"
  local t a; t="$(_actas_lock_encode "$team")"; a="$(_actas_lock_encode "$agent")"
  printf '%s/boot-pid.%s__%s' "$(_actas_lock_dir)" "$t" "$a"
}
