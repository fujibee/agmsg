...
# generated by `spawn.sh` writes its own pid here right before exec'ing the
# role's CLI, and removes it right after that CLI exits. Present iff a
# launcher-spawned CLI process for that role is currently running --
# independent of the ready sentinel above, which only
