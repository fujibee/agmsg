...
4. **Pre-flight claim** the actas exclusivity lock so this role isn't already owned by another live session: `~/.agents/skills/__SKILL_NAME__/scripts/actas-claim.sh "$(pwd)" claude-code <name> "$CLAUDE_CODE_SESSION_ID"`. Read the `status=` line of the output:
    ...
    - `status=held team=<team> owner=<sid> [rollback=incomplete locked=<pairs>]`: another live session currently owns `<name>` in `<team>`. Always abort — do NOT TaskStop, relaunch, or otherwise touch the running Monitor. If `rollback=incomplete` is absent, tell the user: "Cannot actas as `<name>` — it is held by session `<sid>` in team `<team>`. Run `/__SKILL_NAME__ drop <name>` in that session first, then retry." If `rollback=incomplete` is present, report the exact `locked=` pairs that remain held and do not remove their files directly; dropping the peer role alone is insufficient because locks claimed earlier in this attempt remain. Restore the failed filesystem/SQLite infrastructure, then retry `actas` or run normal session cleanup so checked release can finish.
    - `status=not_registered`: shouldn't happen if step 3 ran; treat as an error.
    - `status=error team=<team> reason=<reason> [rollback=incomplete locked=<pairs>]`: abort immediately — do NOT TaskStop, relaunch, or otherwise touch the running Monitor. Report the reason. If `rollback=incomplete` is present, also report the exact `locked=` pairs that remain held; do not remove their files directly. For `reason=legacy-reclaim-marker`, stop and restart every process that could still run the old agmsg mutator, inspect the exact `.reclaim.d` paths reported by SessionStart, remove only each validated affected empty directory, then retry; never age-delete a marker. For any other reason, retry only after the reported filesystem/SQLite infrastructure problem has recovered.
   - **Protocol upgrades and rollbacks:** Do not operate old and new reclaim mutators together. Before the first new-protocol mutation, stop every old agmsg agent, watcher, hook, and lock helper, upgrade them consistently, then restart. Before rolling back code, quiesce every new-protocol helper and process, retain `run/actas-reclaim.db` for inspection, and do not delete it to force the rollback. Marker detection is diagnostic and fail-closed, not proof that the system is quiescent.
