...
**C) Not in a team:**
`not_joined=true available_teams=<t1,t2,...> suggested_team=<team> suggested_agent=<agent>` (or `available_teams=none`)
→ Show the user the available teams from the output, then:
  ...
  1. Use `suggested_team` and `suggested_agent` as the default recommendation.
  2. Ask whether to join using the suggested identity, or choose different names if needed.
...
`suggest=true agents=<n1,n2,...> teams=<t1,t2,...> type=antigravity project=<path> available_teams=<t1,t2,...> suggested_team=<team> suggested_agent=<agent>`
→ No exact registration exists for this project, but there are same-type agent names registered elsewhere.

  1. Show the existing reusable identities and the computed `suggested_team` / `suggested_agent`.
  2. Recommend the computed identity by default.
  3. Ask whether to join using the computed identity, or choose different names if needed.
