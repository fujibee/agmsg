...
`despawn` は `spawn` したメンバーの論理ロールを解除する。通常のtmuxでは、
その後に対象のペインまたはウィンドウを手動で閉じる必要がある。

```
/agmsg despawn reviewer          # グレースフル: ロールを解除し、tmuxのペイン/ウィンドウは手動で閉じる
/agmsg despawn alice --force     # 強制: 記録済みの配置を使うローカル復旧
```

デフォルトで `despawn <name>` は**グレースフル**だ — `<name>` に `ctrl:despawn` 制御メッセージを送信し、その専用ウォッチャーが論理ロールの解除を試みる。tmux配置では意図的にtmuxを操作しないため、対象のペイン/ウィンドウは手動で閉じる。`--timeout <secs>`（デフォルト30）を上限にロールロックが解放済み（free）と観測されるまで待ち、`status=ok ... note=role-lock-free-close-manually` を表示する。ここでの `ok` が確認するのはロックの状態だけで、登録削除の完了確認やtmuxのcloseは保証しない。tmux以外の配置では、この注記なしの `status=ok` のまま。ライフサイクル全体の完了確認は #514 で追跡中。メンバーのウォッチャーが応答しない場合は `status=timeout` を表示して終了コード3 — `--force` で再試行すること。

ただし、記録済みの配置IDが現在の `herdr` ペインと完全に一致する場合は、既存のガード付きクローズ経路でそのペインを閉じる処理を試みる。

`--force` は明示的なローカル復旧用の非常手段だ。メッセージ送信をスキップし、spawn時に記録された配置IDを信頼して、記録済みのペイン/ウィンドウを閉じ、登録を削除する。このIDは認証も世代への紐付けもされていないため、古い、または再利用された配置IDが別のペイン/ウィンドウを対象にしてしまう危険がある。強化は #514 で追跡中。メンバーのウォッチャーが応答できない場合（ウォッチャーが死んでいる、または**codex**メンバー — Monitorがないためグレースフルには何も反応するものがない）に使う。手動で起動されたメンバー（spawnの配置記録がない）は `--force` できない — despawnがその旨を伝え、あなた自身が閉じることになる。

グレースフル経路では、指定メンバー専用のウォッチャーだけが作用する — `despawn` を実行しているセッション自体は終了せず、広い購読範囲を持つウォッチャーは別のロール宛の `ctrl:despawn` を無視する。
...
/agmsg despawn <name> [--force]         — spawnしたメンバーのロールを解除（グレースフル。--forceは復旧用）
