...
`despawn` は `spawn` したメンバーの論理ロールを解除する。配置プロバイダーに
かかわらず、その後に対象のペインまたはウィンドウを手動で閉じる必要がある。

```
/agmsg despawn reviewer          # グレースフル: ロールを解除し、対象のペイン/ウィンドウは手動で閉じる
/agmsg despawn alice --force     # 強制: 記録済みの配置を使うローカル復旧
```

デフォルトで `despawn <name>` は**グレースフル**だ — `<name>` に `ctrl:despawn` 制御メッセージを送信し、その専用ウォッチャーが論理ロールの解除を試みる。tmux、herdr、その他の配置プロバイダーを意図的に操作しないため、対象のペイン/ウィンドウは手動で閉じる。`--timeout <secs>`（デフォルト30）を上限にロールロックが解放済み（free）と観測されるまで待ち、空でない配置記録がある場合は `status=ok ... note=role-lock-free-close-manually` を表示する。ここでの `ok` が確認するのはロックの状態だけで、登録削除の完了確認やペイン/ウィンドウのcloseは保証しない。配置記録のない起動では、この注記なしの `status=ok` のまま。ライフサイクル全体の完了確認は #514 で追跡中。メンバーのウォッチャーが応答しない場合は `status=timeout` を表示して終了コード3 — `--force` で再試行すること。
