...
/agmsg doctor --redacted       # パスと名前を伏せた同じレポート — そのままissueに貼れる
```

素の `doctor` が通常形だ: `claude doctor` や `brew doctor` と同じく、スコープを取らず全体を報告する。`--project <path>`・`--type <type>`・`--team <team>` はレポートを絞り込むもので、自由に組み合わせられる — 疑わしい箇所に当たりがついているときに使うのであって、doctorが要求するものではない。`--redacted` はプロジェクトパスとチーム/エージェント名を一貫した仮名（`<project1>`、`team1/agent2`）に置き換えるので、マシンのレイアウトを漏らさずにバグ報告へ添付できる。
