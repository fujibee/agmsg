...
the value is not understood. Its absence means the client predates the header
(older than the release that introduced it); a client that sends it but could
not determine its own version sends the literal value `unknown` rather than
omitting the header or sending an empty one, so a server can tell "announced,
but unreadable" apart from "never announces a version at all." The value, when
present, is at most 64 printable ASCII characters.
