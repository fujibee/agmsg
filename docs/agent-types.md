...
| `prompt_arg` | — | for a CLI that does not accept the actas prompt as a bare positional argument, the named flag whose value IS the prompt (e.g. antigravity's `--prompt-interactive`, copilot's `--interactive`, opencode's `--prompt`) — `spawn.sh` inserts it immediately before the (already-quoted) prompt. The value may also be a bare `--` end-of-options separator (devin) for a CLI whose positional prompt must be fenced off from option/subcommand parsing. Unset = bare positional, the default |
