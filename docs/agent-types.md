...
| `cli` | spawnable types | the launch command. Usually a single binary name, but may be a fixed multi-word prefix (subcommand and/or flags a CLI needs ahead of its own options, e.g. `opencode run --interactive`) — only the first word is resolved/checked as the executable; the rest are passed through as-is. `spawn` normally leaves that first word late-bound so the child shell can select its PATH wrapper/function/shim. If the parent cannot find it on PATH, `spawn` secures a known-location fallback (`~/.local/bin`, the Windows npm bin, Homebrew, then system bins); the child still gets one final PATH lookup before that absolute fallback is used. Safe because this is manifest data agmsg ships, not runtime user input |
