...
if [ "$UPDATE_ONLY" = true ]; then
  ...
  echo "    It either exits on a half-written script, or keeps running the code"
  echo "    from before the update while reporting itself as running (#963)."
  echo "    Restart it once per team you sync remotely:"
  echo ""
  echo "      remote.sh sync start <team>"
  echo ""
  echo "    'remote.sh status <team>' names the state it is actually in."
  echo ""
