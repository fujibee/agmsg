...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      ...
      echo "                    Types with their own skill file (claude-code, antigravity,"
      echo "                    opencode, hermes, grok-build): writes that file for <t>;"
      echo "                    the shared SKILL.md keeps its type (codex when new)."
      echo "                    Other types (codex, gemini, cursor, devin): selects the"
      echo "                    template the shared SKILL.md is rendered from."
