...
if [ "$UPDATE_ONLY" = true ]; then
  ...
  echo ""
  echo "  ! A running sync engine does NOT detect it, and nothing restarts it."
  echo "    Check each team you sync remotely:"
  echo ""
  echo "      remote.sh status <team>"
  echo ""
  echo "    'engine stale' means it is not running any more, whatever ended it;"
  echo "    start it:"
  echo ""
  echo "      remote.sh sync start <team>"
  echo ""
  echo "    'engine running' after an update means it may still be running the"
  echo "    code from before it. There is no supported way to replace a running"
  echo "    engine yet (#963); stopping the process it names is the only one."
  echo ""
